...
function Update() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://localhost:3000/crud/get/" + id);
                const medtype = response.data.medtype;
                ...
                if (medtype == 'tablet') {
                    ...
                    setExp(format(response.data.expdate,'dd-MM-yyyy'))
                    setNop(response.data.nop)
                    console.log(format(response.data.expdate,'dd-mm-yyyy'))
                    // setSpp(response.data.spp)
                    setPps(response.data.pps)
                }else if (medtype == 'syrup') {
                    setMedtype(medtype)
                    setBatch(response.data.batch)
                    setMedname(response.data.medName)
                    setExp(format(response.data.expdate,'dd-MM-yyyy'))
                    setq(response.data.quantity)
                    setnob(response.data.nob)
                }
    ...
    //     e.preventDefault()
    //     axios.put('http://localhost:3000/crud/update/'+id, {medName , mfgdate , expdate , nop , spp , pps})
    //     .then(res => {
    //         console.log(res);
    //         navigate('/filter')
    //     })
            ...
            <div className='main'>
                <div className='signin-div2'>
                    <div className="top">
                        ...
                        <div  className="idno" ><input type="text" name="batch" id="batch" value={batch} disabled onChange={(e) => setBatch(e.target.value)} /></div>
                        <div className='username' style={{display:'none'}}><input type="text" name="medtype" id="medtype" placeholder="Enter name" value={medtype} onChange={(e) => setMedtype(e.target.value)} /><br /></div>
                    </div>
                        ...
                        <form >
                            ...
                            <div className="fields2" style={{ display: 'flex' }}>
                                <div className='left'>
                                    ...
                                    <div className='username'><input type="text" name="name" id="name" disabled placeholder="Enter name" value={medName} onChange={(e) => setMedname(e.target.value)} /><br /></div>
                                    <label className="med-labels" htmlFor="expiryDate">Expiry Date</label>
                                    <div className='username'><input type="text" name="expdate" id="expdate" placeholder="Pick date" disabled value={expdate} onChange={(e) => setExp(e.target.value)} /><br /></div>
                                </div>
                                <div className='right'>
                                    <label className="med-labels" htmlFor="numberOfPacks">Number of Packet</label><br />
                                    <div className='username'><input type="text" name="numberOfPacks" id="numberOfPacks" placeholder="Enter number" value={nop} onChange={(e) => setNop(e.target.value)} /><br /></div>
                                    {/* <label className="med-labels" htmlFor="nob">Strips per Packet</label><br />
                                    <div className='username'><input type="text" name="stripsPerPacket" id="stripsPerPacket" placeholder="Enter number" value={spp} onChange={(e) => setSpp(e.target.value)} /><br /></div> */}
                                    <label className="med-labels" htmlFor="pillsPerStrip">Pills per Strip</label><br />
                                    <div className='username'><input type="text" name="pillsPerStrip" id="pillsPerStrip" placeholder="Enter number" value={pps} onChange={(e) => setPps(e.target.value)} /><br /></div>
                                </div>
                            </div>
                             )} 
                            ...
                            {medtype === 'syrup' && (
                                <div className="fields3" style={{ display: 'flex' }}>
                                    <div className='left'>
                                        <label className="med-labels" htmlFor="name">Medicine Name</label><br />
                                        <div className='username'><input type="text" name="name" id="name" placeholder="Enter name" disabled value={medName} onChange={(e) => setMedname(e.target.value)} /><br /></div>
                                        <label className="med-labels" htmlFor="expiryDate">Expiry Date</label>
                                        <div className='username'><input type="text" name="expdate" id="expdate" placeholder="Pick date" disabled value={expdate} onChange={(e) => setExp(e.target.value)} /><br /></div>
                                    </div>
                                    <div className='right'>
                                        <label className="med-labels" htmlFor="quantity">Quantity of each bottle</label><br />
                                        <div className='username'><input type="text" name="quantity" id="quantity" placeholder="Enter number" value={quantity}  onChange={(e) => setq(e.target.value)} /><br /></div>
                                        <label className="med-labels" htmlFor="nob">No. of bottles</label><br />
                                        <div className='username'><input type="text" name="nob" id="nob" placeholder="Enter number" value={nob}  onChange={(e) => setnob(e.target.value)} /><br /></div>
                                        {/* <label className="med-labels" htmlFor="pillsPerStrip">Pills per Strip</label><br />
