...
function PrescriptionDetailsPage() {
    ...
    useEffect(() => {
        const fetchPrescriptionDetails = async () => {
            try {
                ...
                console.log(response.data.opno)
                setOp(response.data.opno)
                setLoading(false);
    ...
    const updateStock = async () => {
        try {
            await axios.post(`http://localhost:3000/crud/updateStock/${id}`, updatedStock);
    ...
    const handleDone = async () => {
        try {
            ...
            setAlert2("Error marking prescription as done");
                setTimeout(() => {
                    setAlert2('');
                }, 3000);
            // Handle error
    ...
    return (
        <div className='flex'>
            <div className='signin-div2'>
                ...
                {!done ? (
                    <button onClick={handleDone} className='signin-button'>Pending</button>
                ) : (
                    <button disabled className='signin-button '>Done</button>
                )}
            </div>
        </div>
    );
