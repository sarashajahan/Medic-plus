...
const Otp = () => {
  ...
  const resendOTP = async  (e) => {
    ...
    try {
      await axios.post('http://localhost:3000/doctor/sendOtp', { email: email })
      .then(res=>{
        if(res.data.status){
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("OTP: ", OTP);
    try {
      const response = await axios.post('http://localhost:3000/doctor/validate-otp', {email:email, otp: OTP });
